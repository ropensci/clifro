...
datatype. See `?'plot,cfDataList,missing-method'` for details on default `clifro`
plotting. The following are examples of some of the plots possible with `clifro`, 
note how the optional `ggtheme` argument changes the look of the plots.
...
![MSL Atmospheric Pressure][mslAtmosPress]

Since the first argument passed to `plot` is a list of different datatypes 
(`cfDataList`), the second argument (`y`) tells the `plot` method which of the 
four dataframes to plot.
...
[mslAtmosPress]: figures/mslAtmosPress.png "MSL Atmospheric Pressure"
